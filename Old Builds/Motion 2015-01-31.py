...
class MotionTester:    
    ...
    def addvid(self, link, chk=False, re=False):
        if os.path.isdir(link):
            ...
            if vid.isOpened():
                ...
                self.vbin.append([link, vid, metadata, "N", []])
                if not chk:
    ...
    def runvid(self, pos):
        if not self.posinbin(pos):
            ...
            atf = time.time()
            mtf = time.clock()
            self.log("  > Completed analysis of "+self.vbin[pos-1][0]+" in "+str(mtf-mti)+" process seconds")
            self.log("    > Actual time taken estimated at "+str(atf-ati)+" seconds")
            self.vbin[pos-1][3] = "Y"
    ...
    def run_dt(self, bvid):
        ...
        while bvid[1].isOpened():
            ...
            if None in frs:
                ...
                img_diff = MotionTester.subtract(frs[0], frs[1], frs[2])
                img_thre = cv2.threshold(img_diff, thresh=10.0, maxval=255.0, type=cv2.THRESH_BINARY)
                img_cont = MotionTester.getcontrast(img_thre[1])
                ...
                if fr % (bvid[2][2]*60) == 0:
                    self.log("  > Progress: at frame "+str(fr)+" ("+fr2ts(fr,bvid[2][2])+"), "+str(len(bvid[4]))+" raw intervals found")
    ...
    def subtract(im1, im2, im3):
        return cv2.bitwise_and(cv2.absdiff(im3, im2), cv2.absdiff(im2, im1))
    ...
    def output(self, outfn):
        ...
        if outfn == "Motion_SaveToCurrentSessionLogFile_UsingStringUsersShouldNotEverCoincidentallyUseAsFileName":
            if len([True for bvid in self.vbin if len(bvid) >= 4 and bvid[3] == "Y"]) > 0:
                ...
                for vid in self.vbin:
                    self.log("Video: "+vid[0])
                    self.log("  Properties: "+str(vid[2][0])+"x"+str(vid[2][1])+", "+str(vid[2][2])+" FPS, length "+vid[2][3])
                    self.log("  Intervals Found:")
                    line = ""
                    for itv in vid[4]:
                        if len(line) + len("["+fr2ts(itv[0], vid[2][2])+"-"+fr2ts(itv[1], vid[2][2])+"]") <= 80:
                            line = line + "["+fr2ts(itv[0], vid[2][2])+"-"+fr2ts(itv[1], vid[2][2])+"]"
                        else:
                            self.log("   "+line)
                            line = ""
            else:
            ...
            if len([True for bvid in self.vbin if len(bvid) >= 4 and bvid[3] == "Y"]) > 0:
                ...
                try:
                    ...
                    outf.write("\n\n\n=============== MOTION ANALYSIS RESULTS ===============")
                    for vid in self.vbin:
                        outf.write("Video: "+vid[0])
                        outf.write("  Properties: "+str(vid[2][0])+"x"+str(vid[2][1])+", "+str(vid[2][2])+" FPS, length "+vid[2][3])
                        outf.write("  Intervals Found:")
                        line = ""
                        for itv in vid[4]:
                            if len(line) + len(fr2t(itv, vid[2][2])) < 79:
                                line = line + fr2t(itv, vid[2][2])
                            else:
                                outf.write("   "+line)
                                line = ""
                    ...
                    print "    > If all else fails, use save without a parameter to save to the log."
